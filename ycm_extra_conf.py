...
def Settings( **kwargs ):
  ...
  if language == 'cfamily':
    ...
    global database
    if not database:
      curlist = filename.split("/");
      curlist.pop(0);
      for i in range(len(curlist) - 1, 0, -1):
        curdir = "/";
        for j in range(0, i):
          curdir = curdir + curlist[j] + "/";
        if p.isdir(curdir + "build"):
          database = ycm_core.CompilationDatabase(curdir + "build")
          break
